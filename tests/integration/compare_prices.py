#!/usr/bin/env python3
"""
Integration script: compare HF dataset prices vs fine-tuned model inference (CLI `price`).

Steps:
1) Load N rows from the Hugging Face dataset split
2) Extract each row's Title and ground-truth "Price is $..." value
3) Call: uv run python src/inference.py price "<Title>"
4) Print expected vs predicted + summary stats
"""
...
# You can override without editing the file by setting:
...
def run_price(title: str) -> float:
    ...
    # Prefer stdout (if the CLI prints the final float), otherwise parse "Result: <float>" from logs (stderr).
